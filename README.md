# 🌍 真实旋转地球（3D 可视化）

一个基于 WebGL / [Three.js](https://threejs.org) 的高逼真 3D 地球，支持鼠标/触摸交互，带真实卫星贴图、**随真实时间推移的昼夜线**、城市灯光、云层、大气辉光、四类卫星轨道、月球公转与全球热门旅游城市标注；可**点击城市查看当地时间与简介**，并通过控制面板自由开关图层、切换实时/演示模式。

采用**分离部署**（`index.html` + `libs/` + `assets/`），经服务器打开：不会黑球，并保留**移动端渐进加载**——首屏只需下载 ~72KB 日面图即可看到地球，其余细节后台补齐，首屏更快。

---

## ✨ 功能特性

| 类别 | 内容 |
|------|------|
| **地球本体** | NASA 卫星日面贴图、地形法线（山脉凹凸）、海洋高光、23.5° 地轴倾角 |
| **真实昼夜线** | 按**当前真实时间**计算太阳直射点（赤纬 + UTC 时刻定经线），昼夜分界线与现实一致；背光面自动亮起真实城市灯火 |
| **实时时钟** | 左上角叠层显示 UTC / 本地时间（秒级刷新）与太阳正午所在经线 |
| **点击城市信息卡** | 点击任意城市 → 弹出信息卡：**当地实时时间**（按时区换算）、经纬度坐标、所属国家、一句话简介 |
| **控制面板** | 右上角图层开关（云层 / 城市灯光 / 大气辉光 / 卫星与轨道 / 月球 / 城市标注 / NBA 球队）、**实时昼夜 ↔ 演示自转**切换 + 自转速度滑块、暂停、重置视角 |
| **云层** | 半透明云层，自转速度略快于地表 |
| **大气** | Fresnel 边缘蓝光，模拟大气层散射辉光 |
| **星空** | 4000 颗程序生成的背景星 |
| **卫星轨道** | 近地卫星（LEO）· 极地卫星 · 同步卫星（GEO）· 大椭圆轨道，均按**开普勒运动**（近地点快、远地点慢）|
| **月球** | 带真实月面贴图，绕地公转 |
| **城市标注** | 87 座全球热门旅游城市，**经典红色水滴定位标**（3D 水滴造型 + 顶亮底暗的红色渐变体 + 高光反光 + 凹陷白色圆孔，轻微上下悬浮动效；自发光渐变让昼夜两面都保持鲜亮）+ 落点柔和脉冲光环 + 名称标签，**背面自动隐藏**；**缩小看全球时隐藏名称、放大靠近后才显示**；选中城市定位标放大、光环变蓝 |
| **标签去重叠** | 密集区域（如美东、欧洲）名称不再互相堆叠：定位标始终显示，但名称做**屏幕空间去重叠**——越正对镜头的标签优先占位，冲突者暂隐为纯定位标；**鼠标悬停任一定位标即浮现其名称**（放大 + 手型光标），选中/悬停项始终优先显示 |
| **NBA 球队标注** | 30 支 NBA 球队主场，与城市标注同款水滴造型但采用**紫罗兰配色**以区分；标签显示球队名，点击弹出球队信息卡（格式：*XXX 球队所在位置是 XXX 州 XXX 市*）。**同城两队（纽约尼克斯 / 布鲁克林篮网、洛杉矶湖人 / 洛杉矶快船）共用同一个定位标，标签每 3 秒轮换一次队名**，点击弹出当前所示球队的信息卡。由控制面板「NBA 球队」图层单独开关，**默认关闭** |

### 两种运行模式
- **实时昼夜（默认）**：地球静止，太阳方向按真实时间实时更新——你看到的昼夜分界线就是此刻地球的真实明暗界线。适合观察「现在哪里是白天、哪里入夜」。
- **演示自转**：地球匀速自转、太阳固定，可用滑块调节转速——经典的地球仪演示效果。

### 标注的城市
覆盖六大洲，共 87 座：

- **亚洲**：北京 · 上海 · 香港 · 台北 · 东京 · 大阪 · 京都 · 札幌 · 首尔 · 曼谷 · 清迈 · 新加坡 · 吉隆坡 · 槟城 · 马尼拉 · 河内 · 胡志明市 · 雅加达 · 新德里 · 孟买 · 班加罗尔 · 加尔各答 · 撒马尔罕 · 伊斯兰堡
- **欧洲**：巴黎 · 伦敦 · 罗马 · 米兰 · 威尼斯 · 佛罗伦萨 · 巴塞罗那 · 马德里 · 里斯本 · 柏林 · 慕尼黑 · 维也纳 · 阿姆斯特丹 · 哥本哈根 · 斯德哥尔摩 · 布拉格 · 布达佩斯 · 雅典 · 伊斯坦布尔 · 莫斯科 · 圣彼得堡
- **北美**：纽约 · 芝加哥 · 底特律 · 迈阿密 · 休斯顿 · 拉斯维加斯 · 洛杉矶 · 旧金山 · 西雅图 · 多伦多 · 蒙特利尔 · 温哥华 · 墨西哥城 · 夏威夷
- **南美**：圣保罗 · 里约热内卢 · 布宜诺斯艾利斯 · 利马 · 波哥大 · 圣地亚哥
- **大洋洲**：悉尼 · 墨尔本 · 布里斯班 · 珀斯 · 阿德莱德 · 堪培拉 · 奥克兰 · 惠灵顿
- **非洲 / 中东**：开罗 · 卡萨布兰卡 · 拉各斯 · 内罗毕 · 达累斯萨拉姆 · 亚的斯亚贝巴 · 约翰内斯堡 · 开普敦 · 迪拜 · 阿布扎比 · 多哈 · 利雅得 · 麦加 · 特拉维夫

---

## 🚀 运行方式

本项目通过 WebGL 加载外部贴图，**必须经由 HTTP(S) 打开**，不能直接双击 `index.html`。

> ⚠️ 直接双击（`file://`）会得到**黑色地球**——浏览器把本地图片视为跨域资源、污染纹理，WebGL 拒绝加载。经服务器打开时贴图与页面同源，不存在此问题。

### 本地预览（任选其一）
在项目目录执行，然后浏览器访问打印出的地址：

```bash
npx http-server .        # Node（无需预装，首次会自动下载）
# 或
python -m http.server    # Python 3
```

访问 `http://localhost:8080/`（端口以命令输出为准；`index.html` 会被默认加载）。

### 部署到服务器
把整个目录上传即可（保持相对结构不变）：

```
index.html
libs/
assets/
```

支持任意静态托管：Nginx / Gitee Pages / GitHub Pages / 腾讯云 COS / Netlify / Vercel 等。建议开启 **gzip / brotli**，`.js` / `.html` 可再减小 ~25%。

---

## 🎮 操作方式

| 操作 | 效果 |
|------|------|
| 拖动 / 单指滑动 | 旋转视角 |
| 滚轮 / 双指捏合 | 缩放（1.4 ~ 45 倍地球半径）；**放大靠近后城市名才显示**，缩小看全球时自动隐藏 |
| 悬停定位标 | 光标变手型，**浮现该定位标名称**（即使因去重叠被暂时隐藏），定位标轻微放大 |
| 点击城市 / NBA 球队 | 弹出信息卡：城市显示当地时间 / 坐标 / 国家 / 简介，NBA 球队显示所在州与城市；选中项光环变蓝并强制显示名称；点击空白处关闭 |
| 右上角控制面板 | 切换运行模式、开关各图层、调自转速度、暂停、重置视角（面板标题栏可折叠，手机端默认折叠） |
| 自动 | 卫星运行、月球公转；演示模式下地球自转 |

> 城市名显隐由相机距离阈值 `LABEL_SHOW_DIST`（`index.html` 中，默认 `2.5`）控制：调大更早显示、调小需放得更大才显示。

> **昼夜线原理**：太阳直射点纬度取当日赤纬近似 `δ ≈ -23.44°·cos(360/365·(dayOfYear+10))`，经度由 UTC 时刻推得 `lon = (12 - UTC小时)·15°`；将该地理点经地球四元数变换为世界方向作为光照方向，被照亮的正是真实的地理位置。城市信息卡的「当地时间」按各城市**标准时区**（未计夏令时）换算。

---

## 📁 文件结构

```
.
├── index.html                    # 主文件（经服务器打开）
├── libs/                         # JS 库
│   ├── three.min.js                  # Three.js r128 引擎
│   ├── OrbitControls.js              # 相机交互控制
│   └── CSS2DRenderer.js              # 城市名 HTML 标签渲染
├── assets/                       # 贴图（1024 优化版，~0.69MB）
│   ├── earth_atmos_1024.jpg          # 日面卫星图（首屏关键路径，~72KB）
│   ├── earth_normal_1024.jpg         # 地形法线
│   ├── earth_specular_1024.jpg       # 海洋高光
│   ├── earth_lights_1024.jpg         # 夜间城市灯光
│   ├── earth_clouds_1024.png         # 云层（含透明通道）
│   └── moon_1024.jpg                 # 月面
└── README.md
```

---

## ⚡ 性能优化说明

针对移动端首屏加载做了两项关键优化：

1. **贴图分辨率下调至 1024**：贴图总量从 ~2.2MB 降到 ~0.69MB；夜灯图由 PNG 转 JPG（735KB → 39KB）。
2. **渐进加载**：优先加载 ~72KB 日面图并立即渲染出地球，其余贴图（法线/高光/夜灯/云层/月球）在后台并行加载、就绪后自动浮现——首屏可见时间大幅提前。

分离部署（外链 `assets/`）正是为了让渐进加载在网络层真正生效：浏览器可以先拿到小的日面图渲染、再并行补齐其余贴图；若把贴图全内联进单个 HTML，则必须整份下载完才能渲染，反而拖慢移动端首屏。

### 进一步提速
- 服务器开启 **gzip / brotli** 压缩，`.js` / `.html` 可再减小 ~25%。
- 若需更快，可把 `assets/` 里的 1024 贴图进一步降到 512（辅助贴图尤其合适）。

> 原始 2048 高清母版可从 Three.js 官方示例（`threejs.org/examples/textures/planets/`）重新获取。

---

## 📱 在手机 / 微信中打开

微信打开的是**网址（URL）**而非本地文件，因此需先托管到 HTTPS：

1. 将整个目录部署到静态托管（Gitee Pages / 腾讯云 COS / Netlify 等），得到 `https://...` 链接。
2. 把链接发到微信点开即可（微信内置浏览器支持 WebGL）。
3. iOS 上若不流畅，点右上角「⋯ → 在浏览器打开」。

> 直接把 `.html` 文件发到聊天窗口**不会**渲染，必须用链接。

---

## 🛠️ 技术栈

- **Three.js r128** — WebGL 渲染引擎
- **OrbitControls** — 相机交互
- **CSS2DRenderer** — HTML 城市标签叠加
- **Raycaster** — 点击拾取城市（射线击中地球 → 就近匹配城市）
- 自定义 **GLSL Shader** — 昼夜混合、城市灯光、大气辉光
- **天文近似公式** — 太阳直射点（赤纬 + UTC 经线）驱动真实昼夜线
- **开普勒方程** — 卫星/月球真实轨道运动

## 📄 贴图来源与许可

贴图来自 Three.js 官方示例（`threejs.org/examples/textures/planets/`），原始数据源为 NASA。Three.js 采用 MIT 许可。本项目仅供学习与演示使用。

## ⚠️ 数据说明

轨道尺度已做适度可视化压缩（尤其月球，真实距离约 60 倍地球半径），并非严格比例；卫星运动遵循开普勒规律但为观赏做了时间加速。
